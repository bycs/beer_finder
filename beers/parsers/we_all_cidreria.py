...
def get_all_we_cidreria(base_url: str, cidr_url: list, beer_url: list):
    all_list = []
    html = get_html(f"{base_url}{beer_url[0]}")
    html2 = get_html(f"{base_url}{cidr_url[0]}")
    all_list += get_assortment_we_cidreria(html)
    all_list += get_assortment_we_cidreria(html2)
    return all_list
