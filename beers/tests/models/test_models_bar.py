...
class BarModelTest(TestCase):
    ...
    def setUpTestData(cls):
        Bar.objects.create(name="Cool Bar", website="https://cool-bar.com")

    def test_model_bar_name_label(self):
        bar = Bar.objects.get(pk=1)
        field_label = bar._meta.get_field("name").verbose_name
        self.assertEquals(field_label, "Name")

    def test_model_bar_website_label(self):
        bar = Bar.objects.get(pk=1)
        field_label = bar._meta.get_field("website").verbose_name
        self.assertEquals(field_label, "Website")

    def test_model_bar_updated_label(self):
        bar = Bar.objects.get(pk=1)
        field_label = bar._meta.get_field("updated").verbose_name
        self.assertEquals(field_label, "Updated")

    def test_model_bar_name_max_length(self):
        bar = Bar.objects.get(pk=1)
        max_length = bar._meta.get_field("name").max_length
        self.assertEquals(max_length, 255)

    def test_model_bar_object_name(self):
        bar = Bar.objects.get(pk=1)
        expected_object_name = "%s" % (bar.name)
        self.assertEquals(expected_object_name, str(bar))

    def test_model_bar_object_repr(self):
        bar = Bar.objects.get(pk=1)
        expected_object_name = "<Bar: %s>" % (bar.name)
        self.assertEquals(expected_object_name, repr(bar))
